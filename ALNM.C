...
/*Structs das janelas (n�o � realmente necessario estarem c� em cima*/
struct win middle={ 5,4,75,47,BLUE,GREEN,BLUE,BLACK,WHITE, "Janela Programa",SIMPLES,NULL };
struct win info={ 10,7,70,40,BLUE,WHITE,BLUE,YELLOW,GREEN, "Informa��es",DUPLO,NULL };
...
void showinfo()
{
	...
	gxy(1,13);
	textcolor(WHITE);
	cprintf("    Esta Aplica��o controla uma Base de Dados de alunos\n\n\r");
	cprintf("   com capacidade para armazenar notas, m�dias, moradas,\n\n\r");
	cprintf("        telefones, etc... A utiliza��o � simples e \n\n\r");
...
void showajuda()
{
	...
	cprintf("\r\n  e pressione ENTER para escolher a op��o. Pode usar as teclas real�adas\r\n");
	...
	switch (*opmenu)
	 {
		...
		case 2:
			...
			switch (sopmenu)
			{
				...
				case 2: cprintf("2. Informa��es ",179);
			}
			c = ((c=getch())==0) ? getch() : c;
			if (c>=49 && c<=50) sopmenu=c-48;
			else
			...
			switch (c)
			{
			 ...
			 case 2:closewin(&menus[2]);showinfo();/*mostrar informa��es*/
					 }
			}
			}
			break;
